...
Este script permite crear una tabla y registrar el avance planificado, el avance reprogramado
y el avance real de todos los proyectos en el SE Suite, ajustados por costos.
**/

/** Se elimina tabla CUSTPROJDATA **/
drop table if exists CUSTPROJDATA;

/** 
Ação 5 do procedimento
Crea tabla CUSTPROJDATA **/
drop table if exists CUSTPROJDATA;
create table CUSTPROJDATA (
...
/** Elimina función saveProjTodayData **/
drop function saveProjTodayData;

/** 
Ação 6 do procedimento
Crea función saveProjTodayData que guarda el avance planificado, el avance reprogramado
y el avance real de los proyectos registrados en el SE Suite, ajustados por costos**/
drop function saveProjTodayData;
create or replace function saveProjTodayData()
returns int
language plpgsql
as
$$
...
records integer;
begin
    insert into CUSTPROJDATA(cd_project, id_project,title, record_date, plan_progress, repl_progress,actual_progress) 
    SELECT
...
/** 
Ação 7 do procedimento
Guardar en la tabla CUSTPROJDATA los avances del día, ajustados por costos **/
select saveProjTodayData();

/** Lista datos guardados en la tabla CUSTPROJDATA **/
select * from CUSTPROJDATA;
